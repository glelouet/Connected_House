...
/*
 ...
 The series contains at max BTN_MAX_PUSHES pushes durations, and 0 is aded at
 the end.
...
public:
  ...
  // get the last series stored, also set hasSeries to false until a new series
  // is stored
  // the returned pointer is only valid until next check().
  byte *pop();
