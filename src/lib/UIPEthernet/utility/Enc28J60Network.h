...
private:
  ...
  static uint16_t setReadPtr(memhandle handle, memaddress position,
                             uint16_t len);
  ...
  friend void enc28J60_mempool_block_move_callback(memaddress, memaddress,
                                                   memaddress);
...
public:
  ...
  static uint16_t chksum(uint16_t sum, memhandle handle, memaddress pos,
                         uint16_t len);
};
